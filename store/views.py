...
def product_list(request):
    ...
    if request.method == 'POST':    
        if form['name'].value() == 'All':
            queryset = Product.objects.all()
        else:
            queryset = Product.objects.filter(name__icontains=form['name'].value())
    context = {
	"form": form,
	"header": header,
	"product": queryset,
...
def create_order(request):
    ...
    if request.method == 'POST':
        ...
        calculate = Product.objects.get(name=forms.cleaned_data['product'])

        if int(calculate.sortno) < int(color):
            messages.Error(request, "Please select qunatity less that available quantity"+ str(calculate.sortno))
        else:
            final_qty = int(calculate.sortno) - int(color)
            Product.objects.filter(name=forms.cleaned_data['product']).update(sortno = final_qty)
